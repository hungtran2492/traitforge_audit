...
contract EntityTrading is IEntityTrading, ReentrancyGuard, Ownable, Pausable {
    ...
    //@sus if the approval is revoked after listing and before buying, the transfer might fail.
    //There is a potential for price manipulation if the seller can re-list the NFT at a different price after a buyer has already decided to purchase it.
    // This issue is mitigated by the requirement that the exact price must be sent with the purchase transaction.
